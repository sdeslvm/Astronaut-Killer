...
protocol AstroGradientProviding {
...
final class AstroGradientContainerView: UIView, AstroGradientProviding {
...
struct AstroWebViewBox: UIViewRepresentable {
    // MARK: - Свойства
    
    @ObservedObject var loader: AstroWebLoader
    
    // MARK: - Координатор
    
    func makeCoordinator() -> AstroWebCoordinator {
        AstroWebCoordinator { [weak loader] status in
    ...
    private func setupContainerView(with webView: WKWebView) {
        let containerView = AstroGradientContainerView()
