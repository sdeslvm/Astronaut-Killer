...
protocol AstroWebLoadable: AnyObject {
    var state: AstroWebStatus { get set }
    func setConnectivity(_ available: Bool)
}

/// Протокол для мониторинга прогресса загрузки
protocol AstroProgressMonitoring {
...
final class AstroWebLoader: NSObject, ObservableObject, AstroWebLoadable, AstroProgressMonitoring {
    // MARK: - Свойства
    
    @Published var state: AstroWebStatus = .standby
...
extension AstroWebLoader: WKNavigationDelegate {
