...
struct AstroGameInitialView: View {
    private var gameResourceURL: URL { URL(string: "https://astronakilga.top/games")! }
    
    var body: some View {
        ZStack {
            Color(hex: "#000")
                .ignoresSafeArea()
            AstroEntryScreen(loader: .init(resourceURL: gameResourceURL))
        }
    }
}

#Preview {
    AstroGameInitialView()
}
